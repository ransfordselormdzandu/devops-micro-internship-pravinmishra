...
![alt text](screenshots/05_09.png)
...
![alt text](screenshots/05_13.png)

---

#### Screenshot 14 — Target group showing at least one healthy target

![alt text](screenshots/05_14.png)
...
![alt text](screenshots/05_15.png)

---

#### Screenshot 16 — EC2 instances list showing two running instances in different Availability Zones

![alt text](screenshots/05_16.png)
...
![alt text](screenshots/05_17.png)

---

#### Screenshot 18 — Proof of a database write through a UI message or database query output

![alt text](screenshots/05_18.png)
...
![alt text](screenshots/05_19.png)

---

#### Screenshot 20 — Target group showing healthy targets after replacement

![alt text](screenshots/05_20.png)

---

#### Screenshot 21 — Evidence that an instance was removed, detached, placed in Standby, or stopped in one Availability Zone

![alt text](screenshots/05_21.png)

---

#### Screenshot 22 — Browser showing that the ALB DNS endpoint still works during the change

![alt text](screenshots/05_22.png)
...
![alt text](screenshots/vpc-example-private-subnets-with-ALB.png)
...
The application was deployed in the AWS us-east-1region within a VPC using the CIDR block 10.0.0.0/16, which spans two Availability Zones.

The network consists of four subnets:
...
The public subnets host the Application Load Balancer and web tier, while the private subnets are reserved for the database tier. An Internet Gateway provides internet access to the public subnets, while a NAT Gateway enables resources in the private subnets to access the internet for outbound connections.

Summarize the ALB and Auto Scaling Group setup.

The Application Load Balancer (ALB), named ha-web-alb, was configured across both public subnets to distribute incoming application traffic.

The ALB forwards requests to an Auto Scaling Group (ASG) named ha-web-asg, which runs Ubuntu 26.02 web instances configured using the Launch Template user data. The instances run Apache and PHP and are automatically configured to connect to the RDS database.

The Auto Scaling Group improves application resilience by maintaining the required number of web instances and automatically replacing any instances that become unavailable. The ALB target group performs health checks to ensure that traffic is routed only to healthy instances.

Summarize the private Multi-AZ RDS setup.

The MySQL RDS database is hosted in **private subnets across two Availability Zones**, keeping it inaccessible directly from the internet. **Multi-AZ deployment** provides a standby database in another Availability Zone and enables automatic failover, improving database availability and reliability.

Summarize the results of both high-availability tests.

Test 1 – EC2 Instance Failure:
A web server instance was stopped to simulate failure. The Auto Scaling Group detected the failure and automatically launched a replacement instance. The application remained available through the ALB, demonstrating automatic recovery.

Test 2 – Application Availability:
Traffic was sent through the ALB while web servers operated across two Availability Zones. The ALB continued routing traffic to healthy instances, keeping the application accessible.

Overall: The tests confirmed redundancy, fault tolerance, and automatic recovery in the HA architecture.
